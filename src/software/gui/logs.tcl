...
proc create_logs {root_window note_book} {
	...
	set logs_win [text $logs_f.logs_win -width 80 -height 20 -yscrollcommand "$logs_f.scroll_y set" ]
				#-xscrollcommand {$w.scroll_x set} ]

	scrollbar $logs_f.scroll_y -command "$logs_f.logs_win yview"
	#scrollbar $w.scroll_x -command {$logs_win xview}
